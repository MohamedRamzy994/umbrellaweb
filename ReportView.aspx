<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ReportView.aspx.cs" Inherits="ReportView" Culture="auto" meta:resourcekey="PageResource1" UICulture="auto" %>
    ...
    <script type="text/javascript">
        ...
        function DoWork() {
            ...
            $('a').unbind('click').bind('click', function () {
                var txt = $(this).text();
                if (txt == 'Visit Summary') {
                    $('#hfLoad').val('1');
                    __doPostBack('lbVisit', '');
                }
                if (txt == 'Vital Sings') {
                    $('#hfLoad').val('1');
                    __doPostBack('lbVitalSigns', '');
                }
                else if (txt == 'Complaints & Diagnosis_ud') {
                    $('#hfLoad').val('1');
                    __doPostBack('lbComplaintDiagnosis_ud', '');
                }
                else if (txt == 'Prescription') {
                    $('#hfLoad').val('1');
                    __doPostBack('lbPrescription', '');
                }
                else if (txt == 'Request') {
                    $('#hfLoad').val('1');
                    __doPostBack('lbRequest', '');
                }
                else if (txt == 'Family History') {
                    $('#hfLoad').val('1');
                    __doPostBack('lbFamily', '');
                }
                else if (txt == 'Chronic Diseases') {
                    $('#hfLoad').val('1');
                    __doPostBack('lbChronic', '');
                }
                else if (txt == 'Drugs') {
                    $('#hfLoad').val('1');
                    __doPostBack('lbDrugs', '');
                }
                else if (txt == 'Surgeries') {
                    $('#hfLoad').val('1');
                    __doPostBack('lbSurgeries', '');
                }
                else if (txt == 'Transfusion') {
                    $('#hfLoad').val('1');
                    __doPostBack('lbTransfusion', '');
                }
                else if (txt == 'Hospital Admission') {
                    $('#hfLoad').val('1');
                    __doPostBack('lbHospital', '');
                }
                else if (txt == 'Images') {
                    $('#hfLoad').val('1');
                    __doPostBack('lbImages', '');
                }
            });
            /// Eng.Moustafa Ebrahem
            /// Eng.Moustafa20@Gmail.com
...
<body class='<%= this.user!=null?this.user.theme:"theme-grey"%>'>
    <form id="form1" runat="server">
        ...
        <uc1:ucGeneralClinicReports runat="server" ID="ucGeneralClinicReports" />
