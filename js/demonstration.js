...
function createOnlineUsers_udtatistic(){
	var $el = $("#online-Users_ud"),
	userData = getUser();
...
if($("#online-Users_ud").length > 0){
	createBalanceStatistic();
	createOnlineUsers_udtatistic();
	$left.on("resizestart", function(){
		$("#online-Users_ud").hide();
		$("#balance").hide();
	});
	$left.on("resizestop", function(){
		$("#online-Users_ud").show().sparkline(getUser(), {
			width: ($left.width() > 200) ? 100 : $left.width() - 100,
			height: '25px',
			enableTagOptions: true
		});
